...
function App() {
  ...
  const handleLogin = async () => {
    try {
      await api.post("/auth/init", {}, { withCredentials: true });
      window.location.reload(); // reload to re-run auth check
    } catch (err) {
  ...
  // Create a separate CommentItem component outside your main component
  const CommentItem = ({ comment, level = 0, onReply }) => {
    // Local state for this specific comment's reply text
    ...
    const handleReply = () => {
      ...
      setIsReplying(true);
      onReply(comment.id, localReplyText).finally(() => {
        setIsReplying(false);
        setLocalReplyText("");
      });
    };
    ...
    return (
      <div className={`group relative p-4 rounded-2xl bg-gradient-to-r from-gray-50 to-gray-100/50 hover:from-gray-100 hover:to-gray-200/50 transition-all duration-300 ${level > 0 ? 'ml-8 mt-4' : ''}`}>
        <div className="flex gap-3">
          ...
          <div className="flex-1">
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={handleReply}
                disabled={isReplying || !localReplyText.trim()}
                className="px-6 py-3 rounded-xl bg-gradient-to-r from-purple-500 to-pink-500 text-white font-medium hover:shadow-lg hover:shadow-purple-500/25 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isReplying ? "Sending..." : "Reply"}
              </button>
            </div>

            {/* Render replies - Don't create new reply inputs for replies */}
            {comment.replies?.data?.length > 0 && (
              <div className="mt-4 space-y-3">
                {comment.replies.data.map(reply => (
                  <div key={reply.id} className={`p-3 rounded-xl bg-gray-50 ${level > 0 ? 'ml-4' : ''}`}>
                    <div className="flex gap-3">
                      <div className="flex-shrink-0">
                        <div className="w-8 h-8 rounded-full bg-gradient-to-r from-blue-400 to-cyan-400 p-0.5">
                          <div className="w-full h-full bg-white rounded-full flex items-center justify-center text-xs font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-cyan-600">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className="text-xs text-gray-500">
                            {reply.timestamp ? new Date(reply.timestamp).toLocaleDateString() : 'Just now'}
                          </span>
                        </div>
  ...
  const handleReply = async (commentId, replyText) => {
    try {
      ...
      await api.post(
        `/instagram/comment/${commentId}/reply`,
        { message: replyText },
        { withCredentials: true }
      );
            ...
            <div className="flex items-center gap-6">
              <div className="hidden md:flex items-center gap-6 text-gray-600">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium">{profile.follows_count}</span>
                </div>
                <div className="flex items-center gap-2">
          ...
          <div className="relative flex flex-col md:flex-row items-center md:items-start gap-8">
            ...
            <div className="flex-1 text-center md:text-left">
              ...
              <div className="grid grid-cols-3 gap-6 mb-8">
                ...
                <div className="text-center">
                  <div className="text-2xl font-bold text-gray-900">{profile.follows_count}</div>
      ...
      {selectedPost && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="relative w-full max-w-2xl max-h-[80vh] bg-white rounded-3xl shadow-2xl overflow-hidden border border-gray-200/50">
            ...
            {/* Comments List */}
            {
              loadingComments ? (
                <div className="p-6 flex items-center justify-center">
                  <div className="w-12 h-12 rounded-full border-4 border-white border-t-purple-500 border-r-pink-500 border-b-orange-400 border-l-transparent animate-spin"></div>
                </div>
              ) :
            ...
            {/* Status Message */}
            {replyStatus && (
              <div className={`mx-6 mb-6 p-4 rounded-xl ${replyStatus.includes("successfully")
                ? "bg-gradient-to-r from-green-50 to-emerald-50 text-green-700 border border-green-200"
                : "bg-gradient-to-r from-red-50 to-rose-50 text-red-700 border border-red-200"
                }`}>
                <div className="flex items-center gap-3">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center ${replyStatus.includes("successfully")
